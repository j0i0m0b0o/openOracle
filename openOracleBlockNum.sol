...
contract openOracle is ReentrancyGuard {
    ...
    function createReportInstance(
        ...
        uint256 settlementTime, //in blocks between last report or dispute and settlement
        uint256 escalationHalt, // when exactToken1Report passes this, the multiplier drops to 100 after
        uint256 disputeDelay, // blocks, increase free option cost for self dispute games
        uint256 protocolFee, //in thousandths of a basis point
        uint256 tbps // blocks per second, in tenths of a block
    ...
    function submitInitialReport(uint256 reportId, uint256 amount1, uint256 amount2) external nonReentrant {
        ...
        status.reportTimestamp = getL2BlockNumber();
        status.reportTimestampUnix = block.timestamp;
...
function disputeAndSwap(uint256 reportId, address tokenToSwap, uint256 newAmount1, uint256 newAmount2) external nonReentrant {
    ReportMeta storage meta = reportMeta[reportId];
    ReportStatus storage status = reportStatus[reportId];
    uint256 blockNum = getL2BlockNumber();
    _validateDispute(reportId, tokenToSwap, newAmount1, newAmount2, meta, status, blockNum);
    ...
    status.currentReporter = payable(msg.sender);
    status.reportTimestamp = blockNum;
    status.reportTimestampUnix = block.timestamp;
    ...
    status.lastDisputeBlock = blockNum;
...
function _validateDispute(
    ...
    require(blockNum >= status.reportTimestamp + meta.disputeDelay, "Dispute too early");
    ...
    returns (uint256 price, uint256 settlementTimestamp)
{
    ...
    if (!status.isSettled && !status.isDistributed) {
        ...
        require(
            blockNum >= status.reportTimestamp + meta.settlementTime,
            "Settlement time not reached"
        );

        uint256 settleTimeUnix = (meta.settlementTime * 10) / meta.tbps;
        //help mitigate network halts
        if (block.timestamp <= status.reportTimestampUnix + settleTimeUnix + 60) {
            // Settlement window is still open, modify state
            status.isSettled = true;
            status.settlementTimestamp = blockNum;
